...
## 6. Motivating example 3 --- Suggesting amounts for donations
...
## EDA for the `college` data of Example 3

Salary increases with years of service, there is a more heterogeneity in the salary of higher ranked professors.
...
In observational settings, we need a model to account for potential confounders.

## What explanatories

Continuous variables typically included as linear terms (first-order approximation if the response $Y$ is of the form $\mathsf{E}(Y \mid \boldsymbol{X}) = f(\boldsymbol{X})$ for $f$ sufficiently smooth.

- More complex functions can be included! But beware of the interpretation.


Categorical variables (as `factor`) with $K$ levels are cast into a series of $K-1$ **dummy** variables.
...
The coefficient $\beta_j$ is the **marginal** effect of $X_j$ after all the other variables have been included in the model, $$\beta_j = \partial \mathsf{E}(Y \mid \boldsymbol{X})/ \partial X_j.$$ 
...
- The ordinary residuals $\boldsymbol{e} = \boldsymbol{y} - \widehat{\boldsymbol{y}}$ contains the difference
...
Since zero off-correlations in normal models amount to independence, the MLE for $\sigma^2$ and $\boldsymbol{\beta}$ are independent.


Provided the $(p+1)$ square matrix $\mathbf{X}^\top\mathbf{X}$ is invertible, the large-sample variance

- of the ordinary least squares estimator is $\mathsf{Var}(\widehat{\boldsymbol{\beta}})=\sigma^2(\mathbf{X}^\top\mathbf{X})^{-1}$ and 
- that of the MLE of the variance is $2\sigma^4/n$.

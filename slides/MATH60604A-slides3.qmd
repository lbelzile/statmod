...
- Each increase of one unit in horsepower increases the average autonomy by $(\beta_1 + \beta_2) + 2\beta_2 \texttt{horsepower}$ miles per gallon.
- The marginal effect of `horsepower` (slope) is $\beta_1 + 2\beta_2 \texttt{horsepower}$, which depends on the value of the explanatory.
...
- of the ordinary least squares estimator is $\mathsf{Var}(\widehat{\boldsymbol{\beta}})=\sigma^2(\mathbf{X}^\top\mathbf{X})^{-1}$,
- of the MLE of the variance is $$\mathsf{Var}(\widehat{\sigma}^2) = 2\sigma^4/n$.
